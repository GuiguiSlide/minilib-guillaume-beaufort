...
if (!livre) return res.status(404).json({ erreur: `Livre
...
return res.status(400).json({ erreur: 'Champs manquants', champs:
manquants });
const nouveau = await livresModel.create({ isbn, titre, auteur, annee,
genre });
...
if (!ok) return res.status(404).json({ erreur: `Livre id:${req.params.id}
introuvable` });
res.status(204).send();
};